...
#
# Author: I. Lafaia, based on code by Saghir L. Bashir 
#
# Important note: the report by Saghir L. Bashir was produced in 1998.
# Therefore, all data reported to years after 1998 are simply projections
# which differ from the data used in this code, updated to 2014.
#
#############################################################################
...
#############################################################################
# CHUNK 6: filtering mortality data with codes in nmllCodes
#############################################################################
...
#############################################################################
# CHUNK 10: add column with death rates
#############################################################################
...
#############################################################################
# CHUNK 11: trying to match Saghir's report
#############################################################################
...
cases1 <- mortalityPop %>%
		...
		filter(between(GroupAge, 2, 26)) %>%
		select(Deaths) %>%
		sum(na.rm = TRUE)

cases2 <- mortalityPop %>% filter(between(Year,1985-2,1985+2)) %>% filter(Country == countryCode)
cases3 <- mortalityPop %>% filter(between(Year,1995-2,1995+2)) %>% filter(Country == countryCode)
